...
| `OMBRE_MCP_REQUIRE_AUTH` | 否 | 有 token 时为 `true` | 是否强制保护 `/mcp`、`/sse`、`/messages`；显式设为 `1` 时若没有 token，服务拒绝启动 |
...
- 远程 MCP 默认在 `OMBRE_MCP_TOKEN` 或 `OMBRE_HOME_READ_TOKEN` 存在时开启 Bearer 鉴权。小家会在服务端请求头中携带 token，浏览器端看不到；其他 MCP 客户端也需配置相同 Bearer token。
